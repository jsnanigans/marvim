-- Root detection utilities using vim.fs
-- Finds project root based on common markers
...
-- Find the root directory of the current buffer
function M.find_root(buf)
  ...
  -- Use vim.fs.find to search upward for root markers
...
-- Get or set the root directory for the current buffer
...
-- Change to the root directory of the current buffer
...
-- Setup autocmd to detect root on buffer enter
function M.setup()
